...
class Events(commands.Cog):
    ...
    async def add_event(self, ctx, name=None, date=None, time=None):
        ...
        if name is None:
            ...
            return
        if date is None:
            await ctx.send(('Did you mean to add a task instead? '
                            'Otherwise please redo command with a date'))
            return
        ...
        if time is None:
            ...
            event = {'Time': date, 'Name': name, 'UserID': ctx.author.display_name,
                     'Guild': ctx.message.guild.name}
            collection.insert_one(event)
            embed = discord.Embed(title=f'Your event has been added to {ctx.message.guild.name}',
                                  color=discord.Color(0xFFFF00))
            embed.add_field(name=name, value=date.strftime('%B %d, %Y'), inline=False)
            embed.set_thumbnail(url=("https://upload.wikimedia.org/wikipedia/"
                                     "commons/a/a2/Calendar_12.png"))
    ...
    @commands.command(aliases=['events', 'calendar'],
                      help="Shows the events on the server's calendar.")
    async def show_events(self, ctx):
        ...
        if collection.find({'Guild': ctx.message.guild.name}).count() == 0:
            embed = discord.Embed(title=f'No Events are on {ctx.message.guild.name}',
                                  color=discord.Color(0xFFFF00), timestamp=ctx.message.created_at)
            embed.add_field(name=('To add events use !addevent and '
                                  'separate name, date, and time using "" '),
                            value='Example: !addevent "name" "date" "time(optional)"',
                            inline=False)
            embed.set_thumbnail(url=("https://upload.wikimedia.org/wikipedia/"
                                     "commons/a/a2/Calendar_12.png"))
            await ctx.send(embed=embed)
        ...
        embed = discord.Embed(title=f'Events on {ctx.message.guild.name}',
                              color=discord.Color(0xFFFF00), timestamp=ctx.message.created_at)
        embed.set_thumbnail(url=("https://upload.wikimedia.org/wikipedia/"
                                 "commons/a/a2/Calendar_12.png"))
        ...
        for event in collection.find({'Guild': ctx.message.guild.name}).sort("Time"):
            embed.add_field(name=event["Name"],
                            value=event["Time"].strftime('%B %d, %Y at %I:%M %p'), inline=False)
        await ctx.send(embed=embed)
        return

    @commands.command(aliases=['delevent', 'delete_event', 'delevents'],
                      help="Deletes an event with the given name or all events.")
    async def del_event(self, ctx, name=None):
        ...
        if name.lower() == "all":
            ...
            embed = discord.Embed(title=('All events have been deleted '
                                         f'from {ctx.message.guild.name}'),
                                  color=discord.Color(0xFFFF00), timestamp=ctx.message.created_at)
            embed.set_thumbnail(url=("https://upload.wikimedia.org/wikipedia/"
                                     "commons/a/a2/Calendar_12.png"))
            embed.set_footer(text=ctx.author.display_name, icon_url=ctx.author.avatar_url)
        ...
        for event in collection.find({'Guild': ctx.message.guild.name}):
            if name.lower() == event["Name"].lower():
                ...
                collection.delete_one({'Time': event["Time"], 'Name': event["Name"],
                                       'UserID': event["UserID"], 'Guild': event["Guild"]})
                return

        embed = discord.Embed(title=f'Event is not listed in {ctx.message.guild.name}',
                              color=discord.Color(0xFFFF00))
        embed.set_thumbnail(url=("https://upload.wikimedia.org/wikipedia/"
                                 "commons/a/a2/Calendar_12.png"))
        await ctx.send(embed=embed)
