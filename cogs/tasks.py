...
# Change CLUSTER and DATABASE to your Cluster and Database names
load_dotenv()
...
class Tasks(commands.Cog):
  ...
  # Sets Up Bot
    ...
    async def add_task(self, ctx, arg1 = None, arg2 = None):
      ...
      task = {'Title': arg1, 'Desc': arg2, 'checked': False, 'UserID': ctx.author.display_name, 'Guild': ctx.message.guild.name}
        
      # Fill out Discord Embeded Message
      embedVar = discord.Embed(title="Your task has been added.", color=0x00ff00)
      embedVar.set_thumbnail(url="https://publicdomainvectors.org/photos/sheikh_tuhin_To-Do_List.png")
    ...
    # Edit Task
    @commands.command(help="Edits a task. 'TitleYouWantToChange' 'NewTitle' 'NewDescription'", aliases = ["edittask", 'editt'] )
    ...
    # Grabs Database Information
      ...
      if arg1 == None:
          await ctx.send('Please redo command with this format: !edit_task "TitleYouWantToChange" "NewTaskTitle" "NewTaskDescription" (quotes included).')
          return 
       
    # Make sure task user wants to edit exists in the server they are using the bot in. 
      if (collection.find({"$and": [ {'Guild': ctx.message.guild.name},{"Title":arg1}]}).count() == 0):  
    ...
    # Delete Task
    @commands.command(help="Deletes a task. Write the Title of the task you wish to delete in quotes.", aliases = ["deltask", "delt", "deletet", "deletetask"])
    ...
    # Grab information from Database
     ...
     #Make Sure user specified title to delete
      if arg1 == None:
          await ctx.send('Please redo command with this format: !del_task "Title of Task You Wish to Delete" (Quotes included).')
          return
    ...
    # Check Off Task
    @commands.command(help="Checks off a task. Write the Title of the task you wish to check off in quotes.", aliases = ["checktask", "checkt"])
    ...
    # Make Sure User wrote a title
      if arg1 == None:
          await ctx.send('Please redo command with this format: !check_task "Title of Task You Wish to Check off" (Quotes included).')
          return
    ...
    #Uncheck task 
    @commands.command(help="Unchecks a task. Write the Title of the task you wish to uncheck in quotes.", aliases = ["unchecktask", "uncheckt"])
    async def uncheck_task(self, ctx, arg1 = None):
      ...
      # Make sure user wrote a title
      if arg1 == None:
          await ctx.send('Please redo command with this format: !uncheck_task "Title of Task You Wish to Uncheck" (Quotes included).')
          return
