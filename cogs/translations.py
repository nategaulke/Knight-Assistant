#For this script to work on your local machine, run this command in the terminal in order to install the google translation API library: pip install googletrans==3.1.0a0
...
class Translations(commands.Cog):
    ...
    async def translate(self, ctx, language, *args):
        ...
        #make sure that the language name is in lowercase (all of the keys in the googletrans library are lowercase)
        lang = language.lower()

        #check that the language specified is actually in the dictionary in the googletrans library (see googletrans documentation)
        if lang not in googletrans.LANGUAGES and lang not in googletrans.LANGCODES:   
            ...
            #get the user's original text by concatenating the "arguments" after the "langauge" variable
