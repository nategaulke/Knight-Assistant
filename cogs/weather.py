...
class Weather(commands.Cog):
    ...
    @commands.command(help=("Shows the current weather information for a specific city. "
                            "Simply type the zip code after \"!weather\". "
                            "Currently, this only works for US cities."))
    async def weather(self, ctx, *, zip_code: str):

        weather_api_key = os.getenv('WEATHER_API_KEY')
        weather_api_base_url = "http://api.openweathermap.org/data/2.5/weather?zip="

        weather_api_complete_url = (weather_api_base_url + zip_code +
        ...
        if x["cod"] != "404":
            ...
            embed = discord.Embed(title="Weather in " + x["name"],
                                  color=ctx.guild.me.top_role.color,
                                  timestamp=ctx.message.created_at)
            embed.add_field(name="Descripition", value=f"**{weather_description}**", inline=False)
            embed.add_field(name="Temperature",
                            value=(f"**now - {round(current_temp,1)}°F "
                                   f"({round(current_temp_celsiuis,1)}°C)\n"
                                   f"low - {round(low_temp,1)}°F "
                                   f"({round(low_temp_celsiuis,1)}°C)\n"
                                   f"high - {round(high_temp,1)}°F "
                                   f"({round(high_temp_celsiuis,1)}°C)**"),
                            inline=False)
